...
pub(crate) fn execute_snippet(code: &str, structured: bool) -> Result<Value, String> {
...
pub(crate) fn execute_auto_assemble(script: &str) -> Result<Value, String> {
    with_lua_runtime(|host| host.execute_auto_assemble(script))
}

impl<'a> LuaHost<'a> {
    fn execute_snippet(&self, code: &str, structured: bool) -> Result<Value, String> {
        execute_lua_code(self.state, self.lua, code, structured)
    }
    ...
    fn call_global(
        ...
        execute_lua_global(self.state, self.lua, function_name, args, structured)
    }

    fn execute_auto_assemble(&self, script: &str) -> Result<Value, String> {
        execute_auto_assemble_inner(self.state, self.lua, script)
    }
}

fn with_lua_runtime<T, F>(callback: F) -> Result<T, String>
where
    F: FnOnce(LuaHost<'_>) -> Result<T, String>,
{
...
fn execute_auto_assemble_inner(
    state: *mut LuaState,
    lua: &LuaApi,
    script: &str,
) -> Result<Value, String> {
...
mod tests {
    ...
    use super::{
        fits_json_safe_integer, json_integer_value, normalize_disable_info_symbols,
