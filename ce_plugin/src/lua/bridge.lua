-- ce-mcp embedded Lua backend command bridge
-- Refactored from the historical docs/ce_mcp_bridge.lua reference implementation.
-- Pipe/timer server lifecycle is intentionally removed; Rust injects and calls dispatch().
...
local function integer_hex_for_type(itype, value)
    if value == nil then return nil end
    if itype == "byte" then return toHex(value, 8) end
    if itype == "word" then return toHex(value, 16) end
    if itype == "dword" then return toHex(value, 32) end
    if itype == "qword" then return toHex(value, 64) end
    return nil
end

local function integer_value_for_response(itype, value)
    if value == nil then return nil end
    if itype == "qword" then
        return toHex(value, 64)
    end
    return value
end

local function pointer_value_for_response(value)
    if value == nil then return nil end
    return toHex(value, targetIs64Bit() and 64 or 32)
end
...
local function cmd_read_integer(params)
    ...
    return {
        success = true,
        address = toHex(addr),
        value = integer_value_for_response(itype, val),
        type = itype,
        hex = integer_hex_for_type(itype, val)
    }
end
...
local function cmd_read_pointer(params)
    ...
    return { 
        ...
        value = pointer_value_for_response(finalValue), 
        path = path 
    }
end
...
local SCAN_SESSION_LIMIT = 250000
...
local function parseAobPattern(pattern)
    ...
    for token in string.gmatch(pattern, "%S+") do
        ...
        elseif token:match("^[%x][%x]$") then
            table.insert(parsed, tonumber(token, 16))
        else
            return nil, "invalid AOB token: " .. tostring(token)
        end
    end
...
local function readByteArray(addr, count)
...
local function patternMatchesBytes(bytes, pattern)
    if not bytes or #bytes ~= #pattern then
        return false
    end
    for i = 1, #pattern do
        local expected = pattern[i]
        if expected ~= nil and bytes[i] ~= expected then
            return false
        end
    end
    return true
...
local function scanKindSupportsNumericOrdering(kind)
    return kind == "byte" or kind == "word" or kind == "dword" or kind == "qword"
        or kind == "float" or kind == "double"
end

local function scanTypeRequiresTargetValue(scanType)
    return scanType == "exact" or scanType == "bigger" or scanType == "smaller"
end

local function stringToSearchPattern(searchStr, wide)
    local bytes = {}

    if wide and utf8 and utf8.codes then
        for _, codepoint in utf8.codes(searchStr) do
            if codepoint <= 0xFFFF then
                table.insert(bytes, codepoint % 0x100)
                table.insert(bytes, math.floor(codepoint / 0x100))
            else
                codepoint = codepoint - 0x10000
                local high = 0xD800 + math.floor(codepoint / 0x400)
                local low = 0xDC00 + (codepoint % 0x400)
                table.insert(bytes, high % 0x100)
                table.insert(bytes, math.floor(high / 0x100))
                table.insert(bytes, low % 0x100)
                table.insert(bytes, math.floor(low / 0x100))
            end
        end
    else
        for i = 1, #searchStr do
            table.insert(bytes, searchStr:byte(i))
            if wide then
    ...
    for i, byte in ipairs(bytes) do
        tokens[i] = string.format("%02X", byte)
    end
...
local function cmd_aob_scan(params)
    ...
    local limit = math.max(1, math.min(tonumber(params.limit) or 100, 10000))
    
    if not pattern then return { success = false, error = "No pattern provided" } end
    
    local results = AOBScan(pattern, protection)
    if not results then return { success = true, count = 0, pattern = pattern, addresses = {} } end
...
local function cmd_scan_all(params)
    ...
    if vtype == "array" then
        local patternText = tostring(value)
        local pattern, err = parseAobPattern(patternText)
        if not pattern then
            return { success = false, error = err }
        end

        local protect = params.protection or "+W-C"
        local results = AOBScan(patternText, protect)
        local entries = {}
        local total = 0

        if results then
            total = results.Count or 0
            for i = 0, math.min(total - 1, SCAN_SESSION_LIMIT - 1) do
                local addrStr = results.getString(i)
                local addr = tonumber(addrStr, 16)
                if addr then
                    local bytes = readByteArray(addr, #pattern)
                    if bytes then
                        table.insert(entries, { address = addr, bytes = bytes })
                    end
                end
            end
            results.destroy()
        ...
        serverState.scan_entries = entries
        serverState.scan_kind = vtype
        serverState.scan_mode = "manual"
        serverState.scan_pattern = patternText
        serverState.scan_value_size = #pattern

        return {
            success = true,
            count = #entries,
            type = vtype,
            truncated = total > SCAN_SESSION_LIMIT,
            limit = SCAN_SESSION_LIMIT
        }
    end
    ...
    elseif vtype == "string" then varType = vtString
    ...
    serverState.scan_kind = vtype
    serverState.scan_mode = "memscan"
    
    return { success = true, count = count, type = vtype, truncated = false, limit = SCAN_SESSION_LIMIT }
end

local function cmd_get_scan_results(params)
    local max = math.max(1, math.min(tonumber(params.max) or 100, SCAN_SESSION_LIMIT))

    if serverState.scan_mode == "manual" then
        if not serverState.scan_entries then
            return { success = false, error = "No scan results. Run scan_all first." }
        end

        local results = {}
        local count = math.min(#serverState.scan_entries, max)
        for i = 1, count do
            local entry = serverState.scan_entries[i]
            table.insert(results, {
                address = toHex(entry.address),
                value = bytesToHexString(entry.bytes)
            })
        end
        ...
        return {
            success = true,
            results = results,
            total = #serverState.scan_entries,
            returned = count,
            type = serverState.scan_kind or "array"
        }
    end
    ...
    return {
        ...
        total = fl.getCount(),
        returned = count,
        type = serverState.scan_kind or "dword"
    }
end
...
local function cmd_next_scan(params)
    ...
    local scanKind = serverState.scan_kind or "dword"

    if serverState.scan_mode == nil then
        return { success = false, error = "No previous scan. Run scan_all first." }
    end

    if scanType ~= "exact" and scanType ~= "increased" and scanType ~= "decreased"
        and scanType ~= "changed" and scanType ~= "unchanged"
        and scanType ~= "bigger" and scanType ~= "smaller" then
        return { success = false, error = "unsupported next scan type: " .. tostring(scanType) }
    end

    if scanTypeRequiresTargetValue(scanType) and value == nil then
        return { success = false, error = "missing value" }
    end

    if (scanType == "increased" or scanType == "decreased" or scanType == "bigger" or scanType == "smaller")
        and not scanKindSupportsNumericOrdering(scanKind) then
        return { success = false, error = "scan_type " .. scanType .. " is only supported for numeric scan types" }
    end

    if serverState.scan_mode == "manual" then
        if scanKind ~= "array" then
            return { success = false, error = "manual scan mode only supports array scans" }
        end
        if not serverState.scan_entries then
            return { success = false, error = "No previous scan. Run scan_all first." }
        end

        if scanType ~= "exact" and scanType ~= "changed" and scanType ~= "unchanged" then
            return { success = false, error = "scan_type " .. scanType .. " is only supported for numeric scan types" }
        end

        local pattern = nil
        if scanType == "exact" then
            local err
            pattern, err = parseAobPattern(tostring(value))
            if not pattern then
                return { success = false, error = err }
            end
        end
        ...
        local nextEntries = {}
        for _, entry in ipairs(serverState.scan_entries) do
            local size = serverState.scan_value_size or #(entry.bytes or {})
            local currentBytes = readByteArray(entry.address, size)
            if currentBytes then
                local keep = false
                if scanType == "exact" then
                    keep = patternMatchesBytes(currentBytes, pattern)
                elseif scanType == "changed" then
                    keep = not bytesEqual(currentBytes, entry.bytes)
                elseif scanType == "unchanged" then
                    keep = bytesEqual(currentBytes, entry.bytes)
                end

                if keep then
                    entry.bytes = currentBytes
                    table.insert(nextEntries, entry)
                end
            end
        end

        serverState.scan_entries = nextEntries
        return { success = true, count = #nextEntries, type = scanKind }
    end
    ...
    return { success = true, count = fl.getCount(), type = scanKind }
end
...
local function instruction_mnemonic(disasm)
    local trimmed = trim_text(disasm)
    if not trimmed or trimmed == "" then return nil end
    local mnemonic = trimmed:match("^(%S+)")
    return mnemonic and string.lower(mnemonic) or nil
end

local function instruction_operand_count(disasm)
    local trimmed = trim_text(disasm)
    if not trimmed or trimmed == "" then return 0 end

    local operands = trimmed:match("^%S+%s+(.+)$")
    if not operands or operands == "" then return 0 end

    local count = 1
    for _ in operands:gmatch(",") do
        count = count + 1
    end
    return count
end

local function instruction_flow_control(mnemonic)
    if not mnemonic then return nil end
    if mnemonic == "call" then return "call" end
    if mnemonic == "jmp" or mnemonic == "ljmp" then return "unconditionalbranch" end
    if mnemonic == "ret" or mnemonic == "retn" or mnemonic == "retf" or mnemonic == "iret" or mnemonic == "iretd" or mnemonic == "iretq" then
        return "return"
    end
    if mnemonic:match("^j") or mnemonic:match("^loop") then
        return "conditionalbranch"
    end
    return "next"
end

local function instruction_is_stack_mnemonic(mnemonic)
    if not mnemonic then return nil end
...
local function instruction_near_branch_target(disasm, mnemonic)
    local flowControl = instruction_flow_control(mnemonic)
    if flowControl ~= "call" and flowControl ~= "conditionalbranch" and flowControl ~= "unconditionalbranch" then
        return nil
    end
    ...
    local operand = trimmed:match("^%S+%s+(.+)$")
    if not operand or operand == "" then return nil end

    operand = trim_text((operand:match("^([^,]+)") or operand))
    if not operand or operand == "" then return nil end
    ...
    if operand:find("%[") then
        return nil
    end

    local ok, resolved = pcall(getAddressSafe, operand)
    if ok and resolved then
        return toHex(resolved)
    end

    local rawHex = operand:match("^0[xX]([0-9A-Fa-f]+)$") or operand:match("^([0-9A-Fa-f]+)$")
    if rawHex then
        local numeric = tonumber(rawHex, 16)
        if numeric then
            return toHex(numeric)
        end
    end
...
local function cmd_get_instruction_info(params)
    ...
    local mnemonic = instruction_mnemonic(disasm)
    local nextAddr = size and (addr + size) or nil
    ...
    return {
        ...
        mnemonic = mnemonic,
        next_address = nextAddr and toHex(nextAddr) or nil,
        op_count = instruction_operand_count(disasm),
        is_invalid = false,
        is_stack_instruction = instruction_is_stack_mnemonic(mnemonic),
        flow_control = instruction_flow_control(mnemonic),
        near_branch_target = instruction_near_branch_target(disasm, mnemonic),
        is_ip_rel_memory_operand = false,
        ip_rel_memory_address = nil
    }
end
...
local function cmd_find_function_boundaries(params)
    ...
    for offset = 0, maxSearch do
        ...
        -- 64-bit non-leaf pattern: push rbx; sub rsp, imm8 (40 53 48 83 ...)
        if is64 and b1 == 0x40 and b2 == 0x53 and b3 == 0x48 and b4 == 0x83 then
            funcStart = checkAddr
            prologueType = "x64_nonleaf"
            break
        end
    end
...
local function cmd_analyze_function(params)
    local addr = params.address
    local maxSearch = math.max(1, math.min(tonumber(params.max_search) or 4096, 16384))
    ...
    return {
        ...
        call_count = #calls,
        calls = calls,
        note = not funcEnd and "Function end not found within search range" or nil
    }
end
...
local function normalizeProtectValue(protect)
    local normalized = tonumber(protect) or 0
    if hasFlag(normalized, PAGE_GUARD) then normalized = normalized - PAGE_GUARD end
    if hasFlag(normalized, PAGE_NOCACHE) then normalized = normalized - PAGE_NOCACHE end
    if hasFlag(normalized, PAGE_WRITECOMBINE) then normalized = normalized - PAGE_WRITECOMBINE end
    return normalized
end

local function protectionToString(protect)
    local normalized = normalizeProtectValue(protect)
    if normalized == PAGE_EXECUTE then return "X" end
    if normalized == PAGE_EXECUTE_READ then return "RX" end
    if normalized == PAGE_EXECUTE_READWRITE then return "RWX" end
    if normalized == PAGE_EXECUTE_WRITECOPY then return "RWX" end
    if normalized == PAGE_READONLY then return "R" end
    if normalized == PAGE_READWRITE then return "RW" end
    if normalized == PAGE_WRITECOPY then return "RW" end
    if normalized == PAGE_NOACCESS then return "NOACCESS" end
    return string.format("0x%X", tonumber(protect) or 0)
end

local function regionReadable(region)
    local normalized = normalizeProtectValue(region.Protect or 0)
    return normalized == PAGE_READONLY
        or normalized == PAGE_READWRITE
        or normalized == PAGE_WRITECOPY
        or normalized == PAGE_EXECUTE_READ
        or normalized == PAGE_EXECUTE_READWRITE
        or normalized == PAGE_EXECUTE_WRITECOPY
end

local function regionWritable(region)
    local normalized = normalizeProtectValue(region.Protect or 0)
...
local function regionExecutable(region)
    local normalized = normalizeProtectValue(region.Protect or 0)
...
local function regionUsable(region)
    local state = tonumber(region.State) or 0
    local protect = tonumber(region.Protect) or 0
    local normalized = normalizeProtectValue(protect)
    return state == MEM_COMMIT and not hasFlag(protect, PAGE_GUARD) and normalized ~= 0 and normalized ~= PAGE_NOACCESS
end

local function pushRegionSummary(target, seen, region)
    local base = tonumber(region.BaseAddress) or 0
    if base == 0 or seen[base] then return end

    seen[base] = true
    table.insert(target, {
        base = toHex(base),
        size = region.RegionSize or 0,
        protection = protectionToString(region.Protect or 0),
        readable = regionReadable(region),
        writable = regionWritable(region),
        executable = regionExecutable(region)
    })
end

local function cmd_get_memory_regions(params)
    local maxRegions = tonumber(params.max) or 100
    local anchors = {
    ...
    for _, region in ipairs(regions) do
        if regionUsable(region) and (tonumber(region.BaseAddress) or 0) ~= 0 then
    ...
    for _, anchor in ipairs(anchors) do
        if #result >= maxRegions then break end
        for _, region in ipairs(usable) do
            local base = tonumber(region.BaseAddress) or 0
            local size = tonumber(region.RegionSize) or 0
            local regionEnd = base + size
            if base <= anchor and anchor < regionEnd then
                pushRegionSummary(result, seen, region)
                break
            end
        end
    end
...
local function cmd_search_string(params)
    ...
    if not searchStr or searchStr == "" then
        return { success = false, error = "No search string" }
    end

    local pattern = stringToSearchPattern(tostring(searchStr), wide)
    local results = AOBScan(pattern)
    if not results then
        return { success = true, count = 0, addresses = {} }
    end
...
local function cmd_dissect_structure(params)
    ...
    local size = math.max(1, math.min(tonumber(params.size) or 256, 2048))
    ...
    local guessOk, guessErr = pcall(function() struct:autoGuess(address, 0, size) end)
    if not guessOk then
        pcall(function() struct:removeFromGlobalStructureList() end)
        return { success = false, error = "autoGuess failed: " .. tostring(guessErr) }
    end
    ...
    for i = 0, count - 1 do
        ...
        if elem then
            ...
            table.insert(elements, {
                ...
                name = (elem.Name and elem.Name ~= "") and elem.Name or string.format("field_%04X", elem.Offset),
...
-- Enum Memory Regions Full: isolated in the Lua bridge via CE's native enumMemoryRegions
local function cmd_enum_memory_regions_full(params)
    local maxRegions = tonumber(params.max) or 500
    ...
    for i, r in ipairs(regions) do
        ...
        table.insert(result, {
            ...
            allocation_protect = r.AllocationProtect or 0,
            size = r.RegionSize or 0,
            state = state,
            protect = prot,
            protect_string = protectionToString(prot),
            type = r.Type or 0,
            is_committed = state == MEM_COMMIT,
            is_reserved = state == MEM_RESERVE,
            is_free = state == MEM_FREE
        })
    end
...
local function cmd_read_pointer_chain(params)
    ...
    return {
        ...
        offsets = offsets,
        final_address = toHex(currentAddr),
        final_value = pointer_value_for_response(finalValue),
        chain = chain
    }
end
